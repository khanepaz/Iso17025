...
async function apiGet() {
  ...
  for (var i = 0; i < candidates.length; i++) {
    try {
      var url = candidates[i] + (candidates[i].indexOf('?') >= 0 ? '&' : '?') + 't=' + Date.now();
      const res = await fetch(url, { cache: 'no-store' });
      if (res.ok) {
        const data = await res.json();
        if (data && (data.users || data.companies)) {
          _dataSource = 'static:' + candidates[i];
          return data;
        }
      }
    } catch (e) {}
...
window.ISOAuth = {
  ...
  siteRoot: siteRoot, siteUrl: siteUrl, getDataSource: getDataSource
};
